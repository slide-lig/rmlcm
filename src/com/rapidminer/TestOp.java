...
public class TestOp extends AbstractDataReader{
...
//	}
	...
	public TestOp(OperatorDescription description) {
		super(description);
...
//			private int debugCount = 0;
			...
			private final LineParser parser = new LineParser();
			{
				...
				parser.setEncoding(Encoding.getEncoding(TestOp.this));

				InputStream stream = null;
				try {
					stream = getParameterAsInputStream(PARAMETER_CSV_FILE);
				} catch (IOException e) {
					throw new UserError(TestOp.this, e, 302, stream, e.getMessage());
				}
				reader = new LineReader(stream, Encoding.getEncoding(TestOp.this));
				
				
				// read first line with content and use it as attribute names if the first line was not read before.
				if (firstRowAreNames && !TestOp.this.attributeNamesDefinedByUser()){
					String line = null;
					try {
						do {
...
//				}
			...
			private final NumberFormat numberFormat = StrictDecimalFormat.getInstance(TestOp.this, true);

			private final DateFormat dateFormat = DateParser.getInstance(TestOp.this);
